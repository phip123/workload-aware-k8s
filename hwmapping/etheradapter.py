...
from hwmapping.model import Arch, Accelerator, Location, Bins, Connection, CpuModel, GpuModel, Disk
...
def create_rockpi(name=None) -> Node:
    ...
    return create_node(name=name,
                       cpus=6, arch='aarch64', mem='999036Ki',
                       labels={
...
def create_coral(name=None) -> Node:
    ...
    return create_node(name=name,
                       cpus=4, arch='aarch64', mem='8167784Ki',
                       labels={
                           'ether.edgerun.io/type': 'sbc',
